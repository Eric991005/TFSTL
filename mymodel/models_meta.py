...
class temporalEmbedding(nn.Module):
    def __init__(self, D):
        ...
        self.ff_te = FeedForward([7,D,D])

    def forward(self, TE, T=3):
        '''
        TE:[B,T,2]
        '''
        dayofweek = torch.empty(TE.shape[0], TE.shape[1], 4).to(device) # [B,T,4]
        timeofday = torch.empty(TE.shape[0], TE.shape[1], T).to(device) # [B,T,3]
        for i in range(TE.shape[0]):
            dayofweek[i] = F.one_hot(TE[..., 0][i].to(torch.int64) % 4, 4)
        for j in range(TE.shape[0]):
            timeofday[j] = F.one_hot(TE[..., 1][j].to(torch.int64) % 3, T)
        TE = torch.cat((dayofweek, timeofday), dim=-1) # [B,T,7]
        TE = TE.unsqueeze(dim=2) # [B,T,1,7]
...
class TFSTL(torch.nn.Module):
    ...
    def forward(self, xl, xh, te, adjgat):
        hat_y, hat_y_l = self.features(xl, xh, te, adjgat)
        hat_y, hat_y_l = self.classifier(hat_y, hat_y_l)
        return hat_y, hat_y_l